...
if($_SERVER['REQUEST_METHOD'] === "POST" ){
    ...
    if (isset($postData['nik'])){
        ...
        if(isset($postData['password'])){ //login
            ...
            $update = $postData["data"]["update"];

            $gaji_dibayar = "";
            $potong_cuti = "";

            if ($update){
                $no_ijin = $postData["no_ijin"];
                ...
                $query = "INSERT INTO $tabel (nip, tgl_ijin, jenis_ijin, keterangan, jam_in, jam_out, gaji_dibayar, potong_cuti, tgl_entry) VALUES ('$nip', '$tgl_ijin', '$jenis_ijin', '$keterangan', '$jam_in', '$jam_out', '$gaji_dibayar', '$potong_cuti', CURRENT_TIMESTAMP)";
            }
        }
                ...
                if (!in_array($date->format('N'), $weekends)) {
                    ...
                    if($filtered_array = array_filter($izin, function($item) use ($date_str) {
                        ...
                        if ($filtered_array = array_filter(
                            $absensi, function($item) use ($date_str) {
                            return substr($item['tgl'], 0, 10) === $date_str;
                        })){
                            // cari time absen
                            $times = array_map(
...
// $url = 'https://api-harilibur.vercel.app/api?month=8&year=2020';
// $data = file_get_contents($url);
// $libur = [];
// if ($data) $libur = json_decode($data, true);
// print_r($libur);
